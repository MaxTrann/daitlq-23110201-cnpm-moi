...
import { createCategoryApi, deleteCategoryApi, getAdminCategoriesApi, updateCategoryApi } from "../utils/productApi";
...
const AdminCategoriesPage = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    const res = editingCategory ? await updateCategoryApi(editingCategory._id, values) : await createCategoryApi(values);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        title: "Trạng thái",
        dataIndex: "isActive",
        render: (value) => <StatusBadge tone={value ? "success" : "danger"}>{value ? "Đang dùng" : "Đã ẩn"}</StatusBadge>,
      },
      {
        title: "Hành động",
        render: (_, record) => (
          <Space wrap>
            ...
            <Button
              className="!rounded-full !border-red-500 !bg-red-500 !text-white hover:!border-red-600 hover:!bg-red-600"
              danger
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-[1.75rem] border border-slate-200 bg-white p-4 shadow-sm">
        <Table rowKey="_id" loading={loading} columns={columns} dataSource={categories} pagination={{ pageSize: 8 }} />
      </div>
